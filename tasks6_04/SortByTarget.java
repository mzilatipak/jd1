...
public class SortByTarget<Train> {
	...
	public Train[] sortByTarget(Train[] trains) {
		for (int i = 0; i < trains.length; i++) {
			for (int j = 0; j < trains.length - 1 - i; j++) {
				...
				if (compareString(trains[j].getTarget(), trains[j + 1].getTarget()) > 0) {
					Train tmp = trains[j];
					trains[j] = trains[j + 1];
					trains[j + 1] = tmp;
				}else if (compareByString(trains[j].getTarget(), trains[j+1].getTarget()) == 0)) {
